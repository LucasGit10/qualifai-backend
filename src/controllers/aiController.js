        ...
        if (conversation.schedulingAttempt?.status === 'proposed') {
            ...
            if (lead.status !== 'qualificado') {
                ...
                if (classification === 'quente') {
                    ...
                    try {
                        logger.info(`[Scheduling] Iniciando agendamento para o lead qualificado ${lead._id}...`);
                        ...
                        if (availableSlots.length > 0) {
                            ...
                            logger.info(`[Scheduling] Horários disponíveis para o lead ${lead._id}:`, availableSlots);
                            conversation.schedulingAttempt = {
                                status: 'proposed',
                                proposedTimes: availableSlots,
                            };
                        } else {
                            logger.warn(`[Scheduling] Sem horários disponíveis para agendamento com o lead ${lead._id}.`);
                        }
                    } catch (scheduleError) {
                        logger.error(`[Scheduling] Erro ao buscar horários para o lead ${lead._id}:`, scheduleError);
                    }

                } else {
                    lead.status = classification === 'morno' ? 'morno' : 'frio';
                }
